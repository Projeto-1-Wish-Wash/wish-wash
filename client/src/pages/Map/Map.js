...
const Map = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/lavanderias')
      ...
      .then(data => {
        const lavanderiasFiltradas = data.lavanderias?.filter(l => l.latitude && l.longitude) || [];
  ...
  useEffect(() => {
    if (!userLocation && currentUser && !currentUser.latitude && !currentUser.longitude) {
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      if (window.L && mapRef.current && !map) {
        try {
          const initialCenter = userLocation ? [userLocation.lat, userLocation.lng] : [-7.2186, -35.8811];
  ...
  useEffect(() => {
    ...
    const newUserMarker = window.L.marker([userLocation.lat, userLocation.lng], { icon: userIcon })
      .addTo(map)
      .bindPopup(popupText);
  ...
  useEffect(() => {
    ...
    try {
      ...
      lavanderias.forEach((lavanderia) => {
        if (!lavanderia || !lavanderia.latitude || !lavanderia.longitude || !lavanderia.id) return;
        ...
        try {
          ...
          const marker = window.L.marker([lavanderia.latitude, lavanderia.longitude], { 
            icon: lavanderiaIcon,
          ...
          marker.on('popupopen', async () => {
            try {
              ...
              const maquinasHTML = generateMaquinasHTMLDirect(maquinas, lavanderia.id);
          ...
          console.error('Erro ao criar marcador para lavanderia:', lavanderia.id, error);
        }
      });
  ...
  }, [map, lavanderias]); // Removido lavanderiasMaquinas das dependências
  ...
  const fetchMaquinasLavanderia = async (lavanderiaId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`❌ Erro ao carregar máquinas (${response.status}):`, errorText);
      }
    } catch (error) {
  ...
  const reservarMaquina = async (maquinaId, lavanderiaId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(errorData.error || `Erro ao reservar máquina (${response.status})`);
      }
    } catch (error) {
  ...
  const generateMaquinasHTMLDirect = (maquinas, lavanderiaId) => {
    try {
      ...
      const htmlMaquinas = maquinas.map((maquina) => {
        ...
        const statusColor = status === 'disponivel' ? '#28a745' : 
                           status === 'em_uso' ? '#dc3545' : '#ffc107';
  ...
  const generateMaquinasHTML = (lavanderiaId) => {
    try {
      ...
      const htmlMaquinas = maquinas.map((maquina, index) => {
